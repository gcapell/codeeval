...
var prefRE = regexp.MustCompile(` *(\d+):\[([^]]+)\](, )?`)

func canSeat(line string) bool {
	chunks := strings.Split(line, ";")
	places := atoi(chunks[0])
	dlx := New(places * 2)

	prefs := prefRE.FindAllStringSubmatch(chunks[1], -1)
	for _, pref := range prefs {
		person := atoi(pref[1])
		for _, chair := range strings.Split(pref[2], ", ") {
			dlx.AddRow([]int{person - 1, places + atoi(chair) - 1})
		}
	}
	return dlx.Search()
}
...
func main() {
	...
	for scanner.Scan() {
		fmt.Println(msg[canSeat(scanner.Text())])
	}
